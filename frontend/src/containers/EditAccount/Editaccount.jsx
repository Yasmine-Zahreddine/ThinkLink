...
import deleteConfirmation from "../../../api/delete-confirmation";
const Editaccount = () => {
  ...
  const deleteUser = async (password) => {
    try {
        ...
        const response1 = await deleteConfirmation(userId, password);

        if (!response1.success) {
            throw new Error(response1.message || "Password verification failed.");
        }

        const response2 = await deleteUserApi(userId);
        ...
        if (response2.success) {
            ...
            throw new Error(response2.message || "Failed to delete account.");
        }
    } catch (error) {
        setMessage(error.message || "An error occurred. Please try again.");
                ...
                {loading ? <Loadingspinner /> : (
                  <>
                    <div className="delete-button-container">
                        {!deleteConfirmation && <button className="button delete-button" onClick={() => setDeleteConfirmation(true)}>Delete My Account</button>}
                        {deleteConfirmation && (
